...
type EditorialTopic = { id: string; topic: string; target_characters: number; status: string; source: string; post_id?: string | null; last_error?: string | null; created_at: string; keywords?: EditorialKeyword[] };
...
type SeoGeoAudit = { warnings: SeoGeoWarning[]; checked_at?: string; content_hash?: string };
...
export function EditorialHome({ back, openArticle }: { back: () => void; openArticle: (id: string) => void }) {
  ...
  const generate = async (topic: EditorialTopic) => {
    ...
    try { const response = await fetch(`/api/editorial/topics/${topic.id}/generate`, { method: "POST" }); const payload = await response.json() as { queued?: boolean; position?: number; error?: string }; if (!response.ok) throw new Error(payload.error); setTopics(current => current.map(item => item.id === topic.id ? { ...item, status: "scheduled" } : item)); }
  ...
  const saveTopic = async (topic: EditorialTopic) => { setMessage(""); try { const response = await fetch(`/api/editorial/topics/${topic.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ topic: topic.topic }) }); const payload = await response.json() as { topic?: EditorialTopic; error?: string }; if (!response.ok || !payload.topic) throw new Error(payload.error); setTopics(current => current.map(item => item.id === topic.id ? payload.topic! : item)); } catch (error) { setMessage(error instanceof Error ? error.message : "Téma se nepodařilo upravit"); } };
          ...
          <section className="editorial-overview-panel surface">
            ...
            <div className="editorial-topic-list">
              {topics.map(topic => <article className={selectedTopic === topic.id ? "selected" : ""} key={topic.id} onClick={() => setSelectedTopic(current => current === topic.id ? "" : topic.id)}>
                <div className="topic-title">{selectedTopic === topic.id ? <input className="topic-edit-input" value={topic.topic} maxLength={500} onClick={event => event.stopPropagation()} onChange={event => setTopics(current => current.map(item => item.id === topic.id ? { ...item, topic: event.target.value } : item))} /> : <h3>{topic.topic}</h3>}<span className={`topic-source ${topic.source}`}>{topic.source === "ai" ? "AI" : "Člověk"}</span><span className="topic-characters">{topic.target_characters.toLocaleString("cs-CZ")} znaků</span></div>
                {selectedTopic === topic.id ? <div className="topic-expanded" onClick={event => event.stopPropagation()}>{Boolean(topic.keywords?.length) && <div className="topic-keywords">{topic.keywords!.map(keyword => <span key={keyword.id}>{keyword.query}</span>)}</div>}<div className="topic-expanded-actions"><button className="save-topic" onClick={() => void saveTopic(topic)}>Uložit téma</button><button className="delete-topic" aria-label="Smazat téma" title="Smazat téma" onClick={() => void deleteTopic(topic)}><X size={18} /></button></div></div> : <div className="topic-actions">{topic.status === "review" ? <button aria-label="Otevřít článek" title="Otevřít článek" onClick={event => { event.stopPropagation(); if (topic.post_id) openArticle(topic.post_id); }}><FileText size={18} /></button> : <button className={automation.enabled ? "automatic" : ""} aria-label="Vygenerovat pomocí AI" title="Vygenerovat pomocí AI" disabled={queueing.has(topic.id) || topic.status === "scheduled" || topic.status === "generating"} onClick={event => { event.stopPropagation(); void generate(topic); }}>{queueing.has(topic.id) || topic.status === "scheduled" || topic.status === "generating" ? <LoaderCircle className="spin" size={18} /> : <Sparkles size={18} />}</button>}{automation.enabled && topic.status !== "review" && <time>{nextGeneration}</time>}</div>}
              </article>)}
...
export function EditorialArticleEditor({ articleId, initialLocale = "cs", onLocaleChange, back }: { articleId: string; initialLocale?: string; onLocaleChange?: (locale: string) => void; back: () => void }) {
  ...
  const [state, setState] = useState<"loading" | "ready" | "saving" | "saved" | "error">("loading"); const [action, setAction] = useState<"" | "translate" | "refresh" | "publish" | "delete">(""); const [message, setMessage] = useState(""); const [sources, setSources] = useState<EditorialSource[]>([]); const [claims, setClaims] = useState<EditorialClaim[]>([]); const editorRef = useRef<HTMLDivElement>(null); const autosaveRef = useRef<number | null>(null);
  ...
  const checkSeoGeo = async () => { setSeoChecking(true); setMessage(""); try { if ((hasPendingRevision || state === "saving") && !(await save("autosave"))) throw new Error("Nejdřív se musí podařit uložit aktuální text."); const response = await fetch(`/api/editorial/articles/${articleId}/locales/${locale}/seo-audit`, { method: "POST" }); const payload = await response.json() as { audit?: SeoGeoAudit; error?: string }; if (!response.ok || !payload.audit) throw new Error(payload.error); setSeoAudit(payload.audit); setSeoStale(false); } catch (error) { setMessage(error instanceof Error ? error.message : "SEO/GEO kontrola selhala"); } finally { setSeoChecking(false); } };
  ...
  return <main className="page-shell editorial-page editorial-editor-page"><div className="editorial-editor-top"><button className="back-button" onClick={back}>← Zpět</button></div>
    ...
    <div className="editorial-editor-layout"><section className="editorial-document surface"><div className="editorial-fields"><label><span>Titulek</span><input value={form.title} onChange={event => update({ title: event.target.value })} /></label><label><span>Perex</span><textarea value={form.excerpt} onChange={event => update({ excerpt: event.target.value })} /></label></div><div className="editorial-toolbar"><button title="Tučně" onClick={() => command("bold")}><Bold size={16} /></button><button title="Kurzíva" onClick={() => command("italic")}><Italic size={16} /></button><button title="Nadpis" onClick={() => command("formatBlock", "h2")}>H2</button><button title="Odrážky" onClick={() => command("insertUnorderedList")}><List size={16} /></button><span>{form.body_md.length.toLocaleString("cs-CZ")} znaků</span></div><div className="editorial-rich-editor" ref={editorRef} contentEditable suppressContentEditableWarning onInput={event => update({ body_md: htmlToMarkdown(event.currentTarget.innerHTML) })} data-placeholder="Text článku…" /></section>
      <aside className="editorial-sidebar"><section className="surface editorial-image-upload"><span>Hlavní obrázek</span>{article.hero_image_url ? <button className="editorial-image-preview" type="button" onClick={() => setImagePreviewOpen(true)} aria-label="Zvětšit náhled obrázku"><img src={article.hero_image_url} alt="" /></button> : <div><ImagePlus size={24} /><small>Zatím bez obrázku</small></div>}<div className="editorial-image-actions"><label>Nahrát obrázek<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={event => { void uploadHero(event.target.files?.[0]); event.target.value = ""; }} /></label>{article.hero_image_url && <button type="button" className="remove-image" aria-label="Smazat obrázek" title="Smazat obrázek" disabled={removingHero} onClick={() => void deleteHero()}><X size={17} /></button>}</div></section><details className="surface editorial-research-card"><summary><span><strong>Zdroje a fakta</strong><small>{claims.filter(claim => claim.status === "verified").length}/{claims.length} ověřeno · {sources.length} zdrojů</small></span><ChevronDown size={17} /></summary><div className="editorial-research-card-body"><div className="editorial-research-claims">{claims.map(claim => <div key={claim.id}><i className={claim.status}>{claim.status === "verified" ? <Check size={12} /> : <AlertTriangle size={12} />}</i><span>{claim.claim_text}</span></div>)}{!claims.length && <p>Bez uložené rešerše.</p>}</div>{sources.length > 0 && <div className="editorial-research-sources-list"><strong>Zdroje</strong>{sources.map(source => <a href={source.url} target="_blank" rel="noreferrer" key={source.id}>{source.title}</a>)}</div>}</div></details><section className="surface editorial-seo-geo"><header><div><h3>SEO/GEO podklady</h3><small>{localeNames[locale] ?? locale.toUpperCase()}</small></div><div className="editorial-seo-actions"><button onClick={() => void checkSeoGeo()} disabled={seoChecking || !!action}>{seoChecking ? "Kontroluji…" : "Zkontrolovat"}</button><button className="refresh" onClick={() => void refreshSeoGeo()} disabled={seoChecking || !!action || !form.body_md}>{action === "refresh" ? "Aktualizuji…" : "Aktualizovat"}</button></div></header>{seoKeywords.length > 0 && <div className="editorial-seo-keywords">{seoKeywords.map(keyword => <span key={keyword.id}>{keyword.query}</span>)}</div>}{seoStale && <p className="editorial-seo-stale"><AlertTriangle size={14} />Kontrola je po úpravách zastaralá.</p>}{seoAudit ? seoAudit.warnings.length ? <div className="editorial-seo-warnings">{seoAudit.warnings.map((warning, index) => <div key={`${warning.location}-${index}`}><AlertTriangle size={14} /><span><b>{warning.location}</b>{warning.message}</span></div>)}</div> : <p className="editorial-seo-ok"><Check size={14} />Bez upozornění</p> : <p className="editorial-seo-unchecked">Tato verze zatím nebyla zkontrolována.</p>}</section><section className="surface editorial-seo"><h3>SEO</h3><label><span>Slug</span><input value={form.slug ?? ""} onChange={event => update({ slug: event.target.value })} /></label><label><span>SEO title</span><input value={form.seo_title ?? ""} onChange={event => update({ seo_title: event.target.value })} /></label><label><span>Meta description</span><textarea value={form.seo_description ?? ""} onChange={event => update({ seo_description: event.target.value })} /></label></section><section className="surface editorial-price"><span>Cena</span><strong>{article.ai_usage?.total_tokens ? formatAiCost(article.ai_usage.estimated_cost_czk) : "nezaznamenáno"}</strong></section><button className="editorial-delete" onClick={() => void remove()} disabled={!!action}>Trvale smazat</button></aside></div>

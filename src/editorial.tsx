...
export function EditorialArticleEditor({ articleId, initialLocale = "cs", onLocaleChange, back }: { articleId: string; initialLocale?: string; onLocaleChange?: (locale: string) => void; back: () => void }) {
  ...
  const save = async (mode: "autosave" | "version") => {
    ...
    try { const bodyMd = editorRef.current ? htmlToMarkdown(editorRef.current.innerHTML) : form.body_md; const response = await fetch(`/api/editorial/articles/${articleId}/locales/${locale}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, body_md: bodyMd, saveMode: mode }) }); const payload = await response.json() as { draft?: EditorialDraft; error?: string }; if (!response.ok) throw new Error(payload.error); if (payload.draft) setForm(payload.draft); if (mode === "version") setHasPendingRevision(false); setState("saved"); setMessage(mode === "version" ? "Verze byla uložena." : ""); await load(locale); }

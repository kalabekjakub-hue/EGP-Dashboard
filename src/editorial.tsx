...
import { AlertTriangle, Bold, Check, ChevronDown, FileText, ImagePlus, Italic, Languages, List, LoaderCircle, Plus, Save, Send, Sparkles, Trash2 } from "lucide-react";
...
export function EditorialHome({ back, openArticle }: { back: () => void; openArticle: (id: string) => void }) {
  ...
  const [topicInput, setTopicInput] = useState(""); const [characters, setCharacters] = useState(2200); const [tab, setTab] = useState<"articles" | "topics">("articles");
  ...
  const load = async () => {
    try { const [articleResponse, topicResponse] = await Promise.all([fetch("/api/editorial/articles"), fetch("/api/editorial/topics")]); if (!articleResponse.ok || !topicResponse.ok) throw new Error(); const articlePayload = await articleResponse.json() as { articles: EditorialArticle[] }; const topicPayload = await topicResponse.json() as { topics: EditorialTopic[]; setupRequired?: boolean }; setArticles(articlePayload.articles); setTopics(topicPayload.topics); setMessage(topicPayload.setupRequired ? "Pro tabuli témat je potřeba aplikovat připravenou Supabase migraci." : ""); setState("ready"); } catch { setState("error"); }
  };
  ...
  const addTopics = async (event: FormEvent) => {
    ...
    try { const response = await fetch("/api/editorial/topics", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ topics: values, targetCharacters: characters }) }); const payload = await response.json() as { error?: string }; if (!response.ok) throw new Error(payload.error); setTopicInput(""); await load(); setTab("topics"); }
  ...
  return <main className="page-shell editorial-page"><button className="back-button" onClick={back}>← Zpět</button>
    <form className="editorial-topic-bar surface" onSubmit={event => void addTopics(event)}><Sparkles size={20} /><textarea value={topicInput} onChange={event => setTopicInput(event.target.value)} placeholder="Napiš téma článku. Více témat můžeš vložit každé na nový řádek…" /><label><span>Délka</span><input type="number" min={500} max={12000} step={100} value={characters} onChange={event => setCharacters(Number(event.target.value))} /><small>znaků</small></label><button type="button" className="suggest" disabled={suggesting} onClick={() => void suggestTopic()}><Sparkles size={17} />{suggesting ? "Navrhuji…" : "Navrhnout AI"}</button><button disabled={state === "saving" || !topicInput.trim()}><Plus size={17} /> Přidat na tabuli</button></form>
    {message && <div className="editorial-message"><AlertTriangle size={17} />{message}</div>}
    <nav className="editorial-tabs"><button className={tab === "articles" ? "active" : ""} onClick={() => setTab("articles")}>Články <b>{articles.length}</b></button><button className={tab === "topics" ? "active" : ""} onClick={() => setTab("topics")}>Tabule témat <b>{topics.length}</b></button></nav>
    {state === "loading" ? <div className="editorial-empty surface">Načítám redakci…</div> : state === "error" ? <div className="editorial-empty surface error">Redakci se nepodařilo načíst.</div> : tab === "articles" ? <section className="editorial-article-grid">{articles.map(article => { const cs = article.translations.find(row => row.locale === "cs") ?? article.translations[0]; const value = cs ? currentValue(cs) : null; return <button className="editorial-article-card surface" key={article.id} onClick={() => openArticle(article.id)}>{article.hero_image_url ? <img src={article.hero_image_url} alt="" /> : <span className="editorial-card-placeholder"><FileText size={25} /></span>}<div><span className={`editorial-status ${article.status}`}>{statusLabel(article.status)}</span><h2>{value?.title || article.source_topic || article.slug}</h2><p>{value?.excerpt || "Článek zatím nemá český perex."}</p><footer><span>{article.translations.length}/24 jazyků</span><b>{value ? versionLabel(value, cs?.locale ?? "cs") : "V1"}</b></footer></div></button>; })}{!articles.length && <div className="editorial-empty surface">Zatím tu nejsou žádné články.</div>}</section> : <section className="editorial-board"><div className="editorial-board-column"><header><span>Čekající témata</span><b>{topics.filter(topic => ["queued", "scheduled", "failed"].includes(topic.status)).length}</b></header>{topics.filter(topic => ["queued", "scheduled", "failed"].includes(topic.status)).map(topic => <article className="surface" key={topic.id}><span className={`topic-state ${topic.status}`}>{statusLabel(topic.status)}</span><h3>{topic.topic}</h3><small>{topic.target_characters.toLocaleString("cs-CZ")} znaků · {topic.source === "ai" ? "AI návrh" : "ručně"}</small>{topic.last_error && <p className="topic-error">{topic.last_error}</p>}<button disabled={generating === topic.id} onClick={() => void generate(topic)}>{generating === topic.id ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}{generating === topic.id ? "Generuji…" : "Vygenerovat nyní"}</button></article>)}</div><div className="editorial-board-column"><header><span>Čeká na kontrolu</span><b>{topics.filter(topic => topic.status === "review").length}</b></header>{topics.filter(topic => topic.status === "review").map(topic => <article className="surface" key={topic.id}><span className="topic-state review">Koncept připraven</span><h3>{topic.topic}</h3><button onClick={() => topic.post_id && openArticle(topic.post_id)}><FileText size={16} /> Otevřít článek</button></article>)}</div></section>}
  </main>;
}

...
**Objednávky:** seznam + detail, humanizovaná timeline, ruční FULFILLED, poznámky k položkám, odkazy na screenshoty/doklady. **Stáhnout vše** stáhne ZIP (faktura, oficiální doklady, screenshoty, technický PDF list, anglické zákaznické PDF). **PDF souhrn** stáhne interní technický list (plné `order_id`, stavy, ISO časy, položky, doklady, poznámky). **PDF EN** stáhne anglické zákaznické potvrzení splnění (vozidlo, `order_id`, položky bez selhání a retry). Karty: dokončená zelená, zpracovává se modrá, selhání červená, Plus tyrkysová, čeká na zpracování žlutá, čeká na platbu šedá, konflikt SPZ oranžová. Dokončená karta s konfliktem zůstává zelená a ukáže jen štítek konfliktu. Levý náhled i archiv `/orders` řadí podle času; selhání a dokončené konflikty se netopují nahoru.
...
├── order-export-api.ts       # ZIP + interní PDF list + anglické zákaznické PDF objednávky
...
| `order-export-api.ts` | ZIP balíček, interní technický PDF list a anglické zákaznické PDF objednávky |

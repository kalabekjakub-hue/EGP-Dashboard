    ...
    configureServer(server: import("vite").ViteDevServer) {
      server.middlewares.use("/api/editorial", async (req, res) => {
        ...
        try {
          ...
          json(res, 500, { error: error instanceof Error ? error.message : "Redakční operace selhala" });
        }
      });
    },
  };
